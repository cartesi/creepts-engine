...
export class EngineRunner {
    ...
    private errorMessage(type: string, info?: any): string {
        switch (type) {
            case GameConstants.ERROR_VERSION_MISMATCH:
                return (
                    'Version mismatch. Engine Version: ' +
                    info.engineVersion +
                    '. Logs Version: ' +
                    info.logsVersion +
                    '.'
            ...
            case GameConstants.ERROR_UPGRADE:
                return (
                    "Can't upgrade the turret '" + info.id + "' with max grade."
                );
            case GameConstants.ERROR_LEVEL_UP:
                return (
                    "Can't level up the turret '" +
                    info.id +
                    "' with max level."
                );
            default:
    ...
    public run(
        logs: LogsObject,
        progressCallback?: (progress: EngineState) => void
    ): EngineState {
        const engine = new Engine(
            this.level.gameConfig,
            this.level.enemiesData,
            this.level.turretsData,
            this.level.wavesData
        );

        if (this.level.engineVersion !== engine.version) {
            throw new Error(
                this.errorMessage(GameConstants.ERROR_VERSION_MISMATCH, {
                    logsVersion: this.level.engineVersion,
        ...
        if (!logs.actions || logs.actions.length === 0) {
            throw new Error(
                this.errorMessage(GameConstants.ERROR_ACTION_ARRAY)
            );
        }
        ...
        for (let i = 0; i < logs.actions.length; i++) {
            ...
            if (
                ...
                action.tick < engine.ticksCounter
            ) {
            ...
            while (engine.ticksCounter < action.tick && engine.lifes > 0) {
                ...
                if (
                    ...
                    engine.ticksCounter % progressIncrement == 0
                ) {
                    progressCallback({
                        timestamp: new Date(),
            ...
            switch (action.type) {
                case GameConstants.ACTION_TYPE_NEXT_WAVE:
                    if (!engine.newWave())
                        result.error = { type: GameConstants.ERROR_NEXT_WAVE };
                    break;
                case GameConstants.ACTION_TYPE_ADD_TURRET:
                    result = engine.addTurret(
                        action.turretType,
                        action.position
                    );
                    break;
                ...
                default:
                    result = {
                        error: {
                            type: GameConstants.ERROR_ACTION_TYPE,
                            info: action.type
                        },
                        success: false
                    };
            ...
            if (result.error)
                throw new Error(
                    this.errorMessage(result.error.type, result.error.info)
                );
        ...
        if (engine.lifes > 0) {
            throw new Error(
                this.errorMessage(GameConstants.ERROR_NO_GAME_OVER)
            );
        }
