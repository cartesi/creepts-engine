    ...
    protected calculateTurretParameters(): void {
        ...
        if (this.grade === 3) {
            this.teleportDistance =
                turretDataAtributes.teleportDistance[this.level - 1];
        } else {
            this.intensity = turretDataAtributes.intensity[this.level - 1];
            this.duration = turretDataAtributes.duration[this.level - 1];
        }

        this.durationTicks = Math.floor(
            GameConstants.RELOAD_BASE_TICKS * this.duration
        );
