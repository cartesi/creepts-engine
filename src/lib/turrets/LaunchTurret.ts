...
export class LaunchTurret extends Turret {
    ...
    protected calculateTurretParameters(): void {
        ...
        this.explosionRange =
            turretDataAtributes.explosionRange[this.level - 1];
        ...
        this.projectileSpeed =
            this.grade === 3
                ? 2 * GameConstants.MORTAR_SPEED
                : GameConstants.MORTAR_SPEED;
    ...
    protected getPathCellsInRange(): { r: number; c: number }[] {
        ...
        for (let i = 0; i < this.engine.enemiesPathCells.length; i++) {
            ...
            if (
                (cell.c >= this.position.c &&
                    cell.c <= this.position.c + this.range) ||
                (cell.c <= this.position.c &&
                    cell.c >= this.position.c - this.range)
            ) {
                if (
                    (cell.r >= this.position.r &&
                        cell.r <= this.position.r + this.range) ||
                    (cell.r <= this.position.r &&
                        cell.r >= this.position.r - this.range)
                ) {
                    cells.push(cell);
    ...
    protected shoot(): void {
        if (this.grade === 2) {
            ...
            if (cells.length > 0 && this.numMines < this.level + 3) {
                ...
                const dx = cell.c + 0.5 - this.x;
                const dy = cell.r + 0.5 - this.y;
            ...
            if (
                enemy.type === GameConstants.ENEMY_RUNNER ||
                enemy.type === GameConstants.ENEMY_FLIER
            ) {
                iterations = 3;
            } else {
                iterations = 2;
            ...
            for (let i = 0; i < iterations; i++) {
                ...
                d = MathUtils.fixNumber(
                    Math.sqrt(
                        (this.x - impactPosition.x) *
                            (this.x - impactPosition.x) +
                            (this.y - impactPosition.y) *
                                (this.y - impactPosition.y)
                    )
                );
            }
