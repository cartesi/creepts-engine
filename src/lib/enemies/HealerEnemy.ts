...
export class HealerEnemy extends Enemy {
    ...
    constructor(creationTick: number, engine: Engine) {
        ...
        this.f =
            GameConstants.HEALER_HEALING_TICKS -
            (creationTick % GameConstants.HEALER_HEALING_TICKS);
    ...
    public update(): void {
        ...
        if (this.healing) {
            ...
            if (
                this.f >= GameConstants.HEALER_HEALING_TICKS &&
                this.l < this.engine.enemiesPathCells.length - 2
            ) {
                this.f = 0;
                this.healing = true;
            }
    ...
    private heal(): void {
        ...
        for (let i = 0; i < this.engine.enemies.length; i++) {
            ...
            if (enemy.id === this.id) {
                ...
                const distanceSquare = MathUtils.fixNumber(
                    (enemy.x - this.x) * (enemy.x - this.x) +
                        (enemy.y - this.y) * (enemy.y - this.y)
                );
                if (
                    distanceSquare <=
                    GameConstants.HEALER_HEALING_RADIUS *
                        GameConstants.HEALER_HEALING_RADIUS
                ) {
                    enemy.restoreHealth();
                }
